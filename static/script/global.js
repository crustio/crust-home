export function runGlobal(THREE) {
  const CITY_RADIUS = 500,
      CITY_MARGIN = 0.5,
      BLINT_SPEED = 0.05,
      HEXAGON_RADIUS = 1, //节点半径
      radius = 220
  let scene, camera, renderer, clock, control
  let earthImg, earthData, earthImgData, sphere,
      earthCtx, earthParticles = new THREE.Object3D(),
  ...
  function createDemoScene() {
    ...
    var material = new THREE.MeshBasicMaterial( { color: 0x141414 } );
  ...
  function createEarthParticles() {
      ...
      for (var i = 0; i < 2; i++) {
          ...
          mat.size = 2 // 球面点半径
          mat.color = new THREE.Color(mainColor)
          mat.map = dotTexture
      ...
      for (let i = 0; i < positions.length; i++) {
          let posi = positions[i],
              ...
              typedArr1 = new Float32Array(posi.positions.length),
              typedArr2 = new Float32Array(size.sizes.length)
          for (let j = 0; j < posi.positions.length; j++) {
              typedArr1[j] = posi.positions[j]
          }
